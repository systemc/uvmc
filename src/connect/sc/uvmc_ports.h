...
// SV DPI import functions

extern "C"
{
// TLM1
void  SV2C_put             (int x_id, const bits_t *bits);
svBit SV2C_try_put         (int x_id, const bits_t *bits);
svBit SV2C_can_put         (int x_id);
void  SV2C_get             (int x_id);
svBit SV2C_try_get         (int x_id, bits_t *bits);
svBit SV2C_can_get         (int x_id);
void  SV2C_peek            (int x_id);
svBit SV2C_try_peek        (int x_id, bits_t *bits);
svBit SV2C_can_peek        (int x_id);
void  SV2C_transport       (int x_id, bits_t *bits);
svBit SV2C_try_transport   (int x_id, bits_t *bits);
void  SV2C_write           (int x_id, const bits_t *bits);
...
void  SV2C_b_transport     (int x_id,
                            bits_t *bits,
                            uint64 delay);
}
...
//
// Defines a "typeless" TLM API interface called by SV via the SV2C DPI import
// methods. Each uvmc_*_ports will inherit from this class, implementing only
// the methods pertaining to the port's interface.
//------------------------------------------------------------------------------

class uvmc_port_base : public virtual sc_interface {

  public:
  uvmc_port_base (const string name,
                  const string lookup,
                  const int mask,
                  uvmc_packer *packer=NULL);

  virtual ~uvmc_port_base();
  ...
  static bool resolve(const string name, const string lookup, const int mask,
                      const string type_name, const int sv_id, int *sc_id);

  public:
  static map<string,int> x_port_names;
  static vector<uvmc_port_base*> x_ports;

  bits_t m_bits[MAX_WORDS];
...
class uvmc_tlm1_port_proxy : public uvmc_port_base
{
  public:
  ...
  uvmc_tlm1_port_proxy(const string name, const string lookup, const int mask,
     uvmc_packer *packer=NULL) : uvmc_port_base(name, lookup, mask, packer) {
  }
  ...
  void notify_blocking_sync_process(op_e op) {
    ...
    if (!blocking_process_h.valid()) {
      ...
      blocking_process_h =
        sc_spawn(sc_bind(&this_type::blocking_sync_process,this), name);
    }
    else
...
class uvmc_tlm2_port_proxy : public uvmc_port_base
{
  public:
  ...
  uvmc_tlm2_port_proxy(const string name, const string lookup, const int mask,
                            uvmc_packer *packer=NULL) :
     uvmc_port_base(name, lookup, mask, packer) { }
...
//------------------------------------------------------------------------------
//
// CLASSES- concreate port proxies. To use, first create an sc_port<> of a
...
#define UVMC_PORT_CTOR(IF_NAME,MASK) \
  ...
  uvmc_##IF_NAME##_port(const string name, \
                        const string lookup, \
                        uvmc_packer *packer=NULL) : \
       proxy_type(name,lookup,MASK,packer), \
...
class uvmc_analysis_port
    : virtual public uvmc_tlm1_port_proxy<T,T,CVRT,CVRT>,
      virtual public sc_port<tlm_analysis_if<T>,0,SC_ZERO_OR_MORE_BOUND >
{
  public:
  typedef tlm_analysis_if<T> if_type;
  typedef sc_port<if_type,0,SC_ZERO_OR_MORE_BOUND> port_type;
  typedef uvmc_tlm1_port_proxy<T,T,CVRT,CVRT> proxy_type;
  ...
  uvmc_analysis_port(const string name, 
       ...
       port_type((string("UVMC_PORT_FOR_analysis") +
...
public:
  ...
  initiator_type m_init;

  uvmc_initiator_socket(const string name, const string lookup,
                          uvmc_packer *packer=NULL) :
                             proxy_type(name,lookup,TLM_FW_NB_TRANSPORT_MASK |
                                                    TLM_BW_NB_TRANSPORT_MASK |
                                                    TLM_B_TRANSPORT_MASK, packer),
                   m_init((string("UVMC_INIT_SOCKET_FOR_") + uvmc_legal_path(name)).c_str()) {
    m_init.bind( *this );
  }
  ...
  virtual void b_transport(typename TYPES::tlm_payload_type &trans, sc_time& delay ) {
    m_init->b_transport(trans,delay);
  }
  ...
  virtual tlm::tlm_sync_enum nb_transport_fw(typename TYPES::tlm_payload_type &trans,
                                             typename TYPES::tlm_phase_type &phase,
                                              sc_core::sc_time& delay) {
    return m_init->nb_transport_fw(trans,phase,delay);
  }
