...
//
// We declare outside uvmc namespace to allows users to specialize outside
// the namespace
//------------------------------------------------------------------------------
...
class uvmc_converter<tlm_generic_payload> {
  ...
  // Won't work for TLM2 nb interfaces used in pipelined protocols for example
  ...
  static void do_pack(const tlm_generic_payload &t, uvmc_packer &packer)
  {
    ...
    //cout << "PACK data_len=" << data_len << " byte_en_len=" << byte_en_len << endl;
  ...
  { 
    ...
    if (curr_data_ptr != NULL) {
      ...
      //cout << "**** - allocating new data buffer of length "
      //     << new_data_len << endl;
      new_data_ptr = (unsigned char *)(new int[new_data_len]);
    }
    ...
    if (curr_byte_en_ptr != NULL) {
      ...
      //cout << "**** - allocating new byte en buffer of length "
      //     << new_byte_en_len << endl;
      new_byte_en_ptr = (unsigned char *)(new int[new_byte_en_len]);
    }
    //cout << "UNPACK new_data_len=" << new_data_len
    //     << " new_byte_en_len=" << new_byte_en_len << endl;
