...
{
  string target(sv_target);
  string port_name(sv_port_name);
  string trans_type(sv_trans_type);
  bool ok;

  if (sv_port_type == UVM_PORT) {
    ok = uvmc_channel_base::resolve(port_name,target,sv_mask,
                                    trans_type,sv_id,sc_id);
  }
  else {
    ok = uvmc_port_base::resolve(port_name,target,sv_mask,
                                 trans_type,sv_id,sc_id);
  }

  if (!ok) {
    cerr << "Error: Can not find binding for port registered with ";
    if (target.compare("")) {
      cerr << "names '" << port_name << "' and '" << target << "'";
    }
    else {
          cerr << "name '" << port_name << "'";
    }
...
int uvmc_resolve_name_pair(string port1_name, string port2_name)
{
  ...
  if (port1 != NULL && port2 == NULL) {
    ...
    cout << "Fatal: Can not find any port with the name '"
  ...
  if (file.is_open()) {
    ...
    while (!file.eof()) {
      ...
      if (!port1_name.substr(0,3).compare("SC:")) {
        ...
        cerr << "Error: reading file '" << filename << "'. First field '" 
      ...
      if (!port2_name.substr(0,3).compare("SC:")) {
        ...
        cerr << "Error: reading file '" << filename << "'. Second field '" 
