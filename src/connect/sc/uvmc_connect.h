...
// side.
//
// When name-based SC port lookup is possible...
// TODO? read side-file
...
uvmc_connect_hier(sc_port<tlm_##IF_TYPE##_if<T>,N,POL>&port, \
  ...
  port_proxy = new uvmc_##IF_TYPE##_port<T>(port.name(),lookup,packer); \
...
uvmc_connect_hier(sc_port<tlm_##IF_TYPE##_if<T>,N,POL>&port, \
  ...
  port_proxy = new uvmc_##IF_TYPE##_port<T,CVRT>(port.name(),lookup,packer); \
  port_proxy->bind(port); \
  return (*port_proxy); \
}
...
template<typename T> \
uvmc_##IF_TYPE##_port<T>& \
uvmc_connect(tlm::tlm_##IF_TYPE##_if<T> &interface, \
             string lookup="", \
             uvmc_packer *packer=NULL) { \
    uvmc_##IF_TYPE##_port<T>* port_proxy; \
    port_proxy = new uvmc_##IF_TYPE##_port<T>(sc_gen_unique_name(#IF_TYPE),\
                                              lookup,packer); \
    port_proxy->bind(interface); \
    return (*port_proxy); \
} \
\
template<class CVRT, \
...
uvmc_connect(tlm::tlm_##IF_TYPE##_if<T> &interface, \
             ...
             uvmc_packer *packer=NULL) { \
    uvmc_##IF_TYPE##_port<T,CVRT>* port_proxy; \
    port_proxy = new uvmc_##IF_TYPE##_port<T,CVRT>(sc_gen_unique_name(#IF_TYPE),\
                                                   lookup,packer); \
    port_proxy->bind(interface); \
    return (*port_proxy); \
}
...
template<typename REQ, \
         typename RSP> \
uvmc_##IF_TYPE##_port<REQ,RSP>& \
uvmc_connect(tlm_##IF_TYPE##_if<REQ,RSP> &interface, \
             string lookup="", \
             uvmc_packer *packer=NULL) { \
    uvmc_##IF_TYPE##_port<REQ,RSP>* port_proxy; \
    port_proxy = new uvmc_##IF_TYPE##_port<REQ,RSP>(sc_gen_unique_name(#IF_TYPE),\
                                                    lookup,packer); \
    port_proxy->bind(interface); \
    return (*port_proxy); \
} \
...
uvmc_connect(tlm_##IF_TYPE##_if<REQ,RSP> &interface, \
    ...
    uvmc_##IF_TYPE##_port<REQ,RSP,CVRT_REQ,CVRT_RSP>* port_proxy; \
    port_proxy = new uvmc_##IF_TYPE##_port<REQ,RSP,CVRT_REQ,CVRT_RSP> \
                       (sc_gen_unique_name(#IF_TYPE),lookup,packer); \
    port_proxy->bind(interface); \
    return (*port_proxy); \
}
...
uvmc_connect_hier(sc_port<tlm_##IF_TYPE##_if<T>,N,POL> &port, \
  ...
  proxy = new uvmc_##IF_TYPE##_port<T>(port.name(),lookup,packer); \
  proxy->bind(port); \
  return (*proxy); \
} \
\
...
uvmc_connect_hier(sc_port<tlm_##IF_TYPE##_if<T>,N,POL> &port, \
  ...
  proxy = new uvmc_##IF_TYPE##_port<T,CVRT>(port.name(),lookup,packer); \
  proxy->bind(port); \
  return (*proxy); \
}
...
uvmc_connect_hier(sc_port<tlm_##IF_TYPE##_if<T,PH>,N,POL> &port, \
  ...
  proxy = new uvmc_##IF_TYPE##_port<T,PH>(port.name(),lookup,packer); \
  proxy->bind(port); \
  return (*proxy); \
} \
\
...
uvmc_connect_hier(sc_port<tlm_##IF_TYPE##_if<T,PH>,N,POL> &port, \
  ...
  proxy = new uvmc_##IF_TYPE##_port<T,PH,CVRT>(port.name(),lookup,packer); \
  proxy->bind(port); \
  return (*proxy); \
}
...
UVMC_CONNECT_TLM2_EXPORT(analysis, TLM_ANALYSIS_MASK)
UVMC_CONNECT_TLM2_EXPORT(write, TLM_ANALYSIS_MASK)
...
uvmc_connect(tlm_##IF_TYPE##_if<T>&intf, string lookup="", \
  ...
  port_proxy = new uvmc_##IF_TYPE##_port<T>( \
                string(sc_gen_unique_name(#IF_TYPE)),lookup,packer); \
...
UVMC_CONNECT_TLM2_IF(analysis)
UVMC_CONNECT_TLM2_IF(write)
               ...
               uvmc_packer *packer=NULL)
{
  ...
  initiator.bind(proxy->m_target); // bind target proxy to initiator socket
  return (*proxy);
}
