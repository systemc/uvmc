...
Topic: Point-to-point only
--------------------------

Only point-to-point UVMC connections are allowed at this time. That is, a
UVMC connect call can be made only once for each port, export, imp, and socket
instance. However, this restriction does not limit your connectivity options.
For example, a ~uvm_tlm_analysis_port<T>~ may drive any number of SC-side
analysis imps or exports. The ~uvmc_connect~ call on the SC side returns a
reference to a proxy port that will drive the specified SC-side analysis
